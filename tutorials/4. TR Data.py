...
3. 위에서 해당 이벤트의 rq_name이 'balance'일 때 Slot을 연결해 두었기 때문에 Server.balance(...) 함수가 동일한 입력값으로 호출됨

참고) 이 때 Server.balance(...) 함수가 호출되었는데 데이터가 남아있어 다시 Signal 함수 요청이 필요하다면
...
class Bot(Bot):
    ...
    def run(self):
        ...
        # 다운로드된 예수금 정보 출력
        print('\n-- 예수금확인 --')
        print(f"예수금 : {format(self.server.data['deposit'], ',')}원")

        # 다운로드된 잔고 데이터 확인
        print('\n-- 계좌잔고확인 --')
        print(f"Single Data: {self.server.data['balance']['single']}")
        print(f"Multi Data: {self.server.data['balance']['multi']}")
...
Bot.deposit() 호출
	Server.deposit(scr_no, rq_name, tr_code, record_name, prev_next, *args) 호출
	Server.deposit(scr_no, rq_name, tr_code, record_name, prev_next, *args) 종료
Bot.deposit() 종료
Bot.balance(prev_next) 호출
	Server.balance(scr_no, rq_name, tr_code, record_name, prev_next, *args) 호출
	Server.balance(scr_no, rq_name, tr_code, record_name, prev_next, *args) 종료
